...
import { getEpitechCalendar, getEpitechCourses } from './Epitech';
import { connectGoogleCalendar, getTokenWithRefresh } from './Google';
import { json } from "./config";
import { sleep } from './Tools';
import { addCalendar, addEventCalendar, removeEventCalendar, eventIsInList } from './Calendar';

async function refreshEpiEventCalendar(calendar: calendar_v3.Calendar, result: calendar_v3.Schema$CalendarList | undefined) {
    return new Promise(async (resolve, reject) => {
        var EpiCalendar: calendar_v3.Schema$Calendar | undefined = result?.items?.find(x => x.summary == "EpiEvent");
        ...
        calendar.events.list({
            ...
            var ScheduleCalendars: calendar_v3.Schema$Event[] | undefined | void = await getEpitechCalendar().catch((error) => resolve(error));
...
async function refreshEpitechCalendar(auth: any): Promise<any> {
    return new Promise((resolve, reject) => {
        ...
        calendar.calendarList.list({}, async (err: any, result) => {
            if (err?.response?.data?.error === "invalid_grant") return getTokenWithRefresh(auth).then((value) => resolve(value)).catch((reason) => reject(reason));
            if (err) return reject(err.response.data);
            await refreshEpiEventCalendar(calendar, result?.data).catch((reason) => reject(reason));
            resolve(true);
        });
    });
}
